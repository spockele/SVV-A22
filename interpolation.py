...
def lagrange(xvals, yvals):    
    def _basis(j):
        p = [(x - xvals[i])/(xvals[j] - xvals[i]) for i in range(n) if i != j]
        return reduce(operator.mul, p) #multiplies all basis functinos together
    
    x = Symbol('x')

    n = len(xvals)
    sigma = sum(_basis(j)*yvals[j] for j in range(n))
    
    LgnPoly = simplify(sigma)
    LgnPoly = Poly(LgnPoly)       
    return LgnPoly.all_coeffs()
