...
#As calculated, we have that the loading along the Span is as follows, corresponding to xarray:
...
for i in range(len(qqq)):
    q = np.append(q,qqq[i]*x**(len(qqq)-i-1))
...
x, R1z, R2z, R3z, Ra1, R1y, R2y, R3y, C1, C2, C3, C4, C5 = symbols('x R1z R2z R3z Ra1 R1y R2y R3y C1 C2 C3 C4 C5')

Sz = R1z*Mac(x-x1,0) + R2z*Mac(x-x2,0) + R3z*Mac(x-x3,0) + cos(theta)*Ra1*Mac(x-xa1,0) - cos(theta)*P*Mac(x-xa2,0)

Sy = -R1y*Mac(x-x1,0) - R2y*Mac(x-x2,0) - R3y*Mac(x-x3,0) + sin(theta)*P*Mac(x-xa2,0)
for i in q:
    Sy = Sy+i

print(Sy)
# Reaction forces
b = [z1, z2, z3, a1, y1, y2, y3, 0, 0, 0, 0, 0]
